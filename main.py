      ...
      while (wayPoint <= hmax):
            ...
            if adcounter%2 != 0:
              ...
              if descendingWayPoint<=startOfBreakPhase:
                ...
                #print(str(time)+" BREAK PHASE")
            #if the drone is really close to its max height, it will slow down a lot so it can easily change directions for the next phase
            '''if(abs(wayPoint-hmax) >=0 and abs(wayPoint-hmax) <=.2):
              velocity = 0.3
            '''
            c6 = velocity
            ...
            #this is a function that calculates time as a function of velocity; currently a comment.
            #time = .5 * (wayPoint - hmin) / abs(velocity)
            #print("The velocity at height " +str(wayPoint)+ " is "+str(velocity) +" and the time is "+str(time))
